...
The default address is `http://127.0.0.1:3000`. `/reports.html` provides whole-report search, questions, JSON export, and table-cell CSV export. `/file-upload.html` starts durable extraction jobs. `/dashboard.html` provides the assessment workspace. Open **Review → Ask about this property** to ask questions against the matching report, or use **Report evidence** in the sidebar.

Keep mutable data outside the checkout and outside synced storage. On macOS the default is `~/Library/Application Support/Project Serafina`. `.env` is local machine configuration and is not versioned. The canonical checkout is `main`, with one application on port 3000 using that default data directory. The former audit workspace has been consolidated into it; do not start a second app on port 3011.
